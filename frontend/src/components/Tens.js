...
const Tens = () => {
    ...
    const [duplicateTens, setDuplicateTens] = useState([]);
        ...
        async function getChanel() {
            ...
            try {
                setTimeout(async () => {
                    ...
                    setTens(response.data.eyeGlass);
                    setDuplicateTens(response.data.eyeGlass);
                    setLoading(false);
                }, 700);
            } catch (error) {
                console.log(error);
    ...
    function filterBySearch() {
        const tempTens = duplicateTens.filter((tens) => tens.model.toLowerCase().includes(searchkey.toLowerCase()));
        setHasResults(tempTens.length > 0);
        setTens(tempTens);
    }

    function filterByType(e) {
        setType(e);
        const filteredTens = applyFilters(e, gender, selectedPriceRange);
        setHasResults(filteredTens.length > 0);
        setTens(filteredTens);
    }

    function filterByGender(e) {
        setGender(e);
        const filteredTens = applyFilters(type, e, selectedPriceRange);
        setHasResults(filteredTens.length > 0);
        setTens(filteredTens);
    }
    ...
    function filterByPrice(e) {
        ...
        const filteredTens = applyFilters(type, gender, e);
        setHasResults(filteredTens.length > 0);
        setTens(filteredTens);
    }

    function applyFilters(selectedType, selectedGender, selectedPrice) {
        return duplicateTens.filter((tens) => {
            const typeCondition = selectedType === 'all' || tens.type.toLowerCase() === selectedType.toLowerCase();
            const genderCondition = selectedGender === 'all' || tens.gender.toLowerCase() === selectedGender.toLowerCase();
            const priceCondition = selectedPrice === 'all' || (tens.price <= parseInt(selectedPrice) && tens.price >= parseInt(selectedPrice) - 9999);
